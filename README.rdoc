...
You have the option of multiple ticket stores, including
+:local_dir_ticket_store+:: Session stored using the local file system
+:active_record_ticket_store+:: Session stored using ActiveRecord store
+:active_model_memcache_ticket_store+:: Session stored using Memcached
+:active_model_redis_ticket_store+:: Session stored using Redis
...
==== Memcached storage of Sessions

For Memcached storage set up the configuration with <tt>ticket_store</tt> and <tt>ticket_store_config</tt>

CASClient::Frameworks::Rails::Filter.configure(
  ...
    ticket_store: :active_model_memcache_ticket_store,
    ticket_store_config: {
      dalli_settings: DALLI_SETTINGS
    }
    ...
)

Set up a ActiveModelMemcacheStore to be able to have the session being created and destroyed with the service_tickets under <tt>config/initializers/active_model_memcache_store.rb<tt>; similar to the one such as templates/active_model_memcache_store.rb

Set up session storage under<tt>config/initializers/session_store.rb</tt>

Rails.application.config.session_store ActionDispatch::Session::ActiveModelMemcacheStore,
  cache: ActiveSupport::Cache::DalliStore.new("#{DALLI_SETTINGS.fetch('host', '')}:#{DALLI_SETTINGS.fetch('port', '')}", DALLI_SETTINGS.symbolize_keys.merge(expire_after: session_expiry))

With <tt>ActionDispatch::Session::ActiveModelMemcacheStore</tt> managing sessions through either session_id or service_ticket. <tt>DALLI_SETTINGS</tt> contains the configuration to access the Memcached server.

==== Redis storage of Sessions

For Redis storage set up the configuration with <tt>ticket_store</tt> and <tt>ticket_store_config</tt>

CASClient::Frameworks::Rails::Filter.configure(
  ...
    ticket_store: :active_model_redis_ticket_store,
    ticket_store_config: {
...
Set up an ActiveModelRedisStore to be able to have the session be created and destroyed with the service_tickets under <tt>config/initializers/active_model_redis_store.rb</tt>; similar to the one in templates/active_model_redis_store.rb

Set up session storage under <tt>config/initializers/session_store.rb</tt>

Rails.application.config.session_store ActionDispatch::Session::ActiveModelRedisStore,
                                           redis_server: { host: REDIS_SETTINGS.fetch('host', ''), port: REDIS_SETTINGS.fetch('port', ''), db: REDIS_SETTINGS.fetch('db', '')},
                                           key: "_session_id",
                                           namespace: REDIS_SETTINGS.fetch('namespace', ''),
                                           expire_after:session_expiry,
                                           secret: SESSION_SECRET_KEY,
                                           secure: Rails.env.production?
                                           
With <tt>ActionDispatch::Session::ActiveModelRedisStore</tt> managing sessions through either session_id or service_ticket. <tt>"REDIS_SETTINGS</tt> contains the configuration to access the Redis server.

The appropriate version of the redis-rails gem will also need to be added to the Gemfile (EVEN IF YOU ARE NOT USING REDIS).
