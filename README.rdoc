...
Set up a ActiveModelRedisStore to be able to have the session being created and destroyed with the service_tickets under <tt>config/initializers/active_model_redis_store.rb<tt>; similar to the one such as templates/active_model_redis_store.rb

Set up session storage under<tt>config/initializers/session_store.rb</tt>

Rails.application.config.session_store ActionDispatch::Session::ActiveModelRedisStore,
  cache: ActiveSupport::Cache::RedisCacheStore.new("#{REDIS_SETTINGS.fetch('host', '')}:#{REDIS_SETTINGS.fetch('port', '')}", REDIS_SETTINGS.symbolize_keys.merge(expire_after: session_expiry))
