shared_examples "a ticket store interacting with sessions" do
  ...
  describe "#store_service_session_lookup" do
    ...
    it "should store the ticket without any errors" do
      allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:find_session).and_return([], ["nonexistant_session", {"session_id" => "ST-id"}])
      allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:write_session).and_return(["nonexistant_session", {"session_id" => "ST-id"}])
  ...
  describe "#get_session_for_service_ticket" do
    context "the service ticket is nil" do
      it "should raise CASException" do
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:find_session).and_return([], ["nonexistant_session", {:session_id => service_ticket}])
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:write_session).and_return(["nonexistant_session", {:session_id => service_ticket}])
    ...
    context "the service ticket is associated with a session" do
      before do
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:find_session).and_return([],[])
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:write_session).and_return([])
      ...
      it "should return the session_id and session for the given service ticket" do
        ...
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:write_session).and_return(true)
      ...
      context "the session" do
        it "should be destroyed" do
          if subject.instance_of?(CASClient::Tickets::Storage::ActiveModelMemcacheTicketStore)
            ...
            allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:find_session).and_return(nil)
            dc.find_session({}, session.session_id).should nil
          else
...
shared_examples "a ticket store" do
  ...
  let(:session) do
    if ticket_store.is_a?(CASClient::Tickets::Storage::ActiveModelMemcacheTicketStore)
      ...
      dc.write_session({}, session_id, cache_data, {})
      cache_data
    else
  ...
  context "when dealing with Proxy Granting Tickets and their IOUs" do
    ...
    describe "#retrieve_pgt" do
      before do
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:find_session).and_return(["session_id_1",{:pgt_iou => pgt_iou, :pgt_id=> pgt}],["session_id_1",{:pgt_iou => pgt_iou, :pgt_id=> pgt}])
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:write_session).and_return(["session_id_1",{:pgt_iou => pgt_iou, :pgt_id=> pgt}],["session_id_1",{:pgt_iou => pgt_iou, :pgt_id=> pgt}])
      ...
      it "should not return the stored pgt a second time" do
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:find_session).and_return(["session_id_1",{:pgt_iou => pgt_iou, :pgt_id=> pgt}],[])
        allow_any_instance_of(ActionDispatch::Session::ActiveModelRedisStore).to receive(:write_session).and_return(["session_id_1",{:pgt_iou => pgt_iou, :pgt_id=> pgt}],[])
        subject.retrieve_pgt(pgt_iou).should == pgt
